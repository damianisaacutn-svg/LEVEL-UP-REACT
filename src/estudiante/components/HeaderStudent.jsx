  ...
  useEffect(() => {
    const getUserData = async () => {
      try {
        ...
        const name =
          profile?.nombre?.trim() || user.user_metadata?.nombre || user.email || 'Usuario'
        ...
        if (adminError && adminError.code !== 'PGRST116') {
          console.error('Error obteniendo rol administrador:', adminError)
        }
        ...
        console.error('Error general en Header:', error)
      } finally {
        setLoading(false)
      }
    }
    ...
    <header className="h-16 bg-white border-b border-[#EAEAEA] fixed top-0 right-0 left-64 z-20 flex items-center justify-between px-8">
      ...
      <div className="flex-1 max-w-xl">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Buscar cursos, módulos o contenido..."
            className="w-full pl-10 pr-4 py-2 bg-[#F5F5F5] rounded-lg text-sm text-[#1A1A1A] outline-none focus:ring-2 focus:ring-[#FF2D55]"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        >
          ...
          {/* Dropdown */}
          {openMenu && (
            <div className="absolute right-0 top-12 w-52 bg-white border border-[#EAEAEA] rounded-xl shadow-lg p-2">
